...
return function(mod_path, request, status_file)
    if love.system.getOS() ~= 'Windows' then
        return love.system.openURL('http://127.0.0.1:8766')
    end
    local ok, result = pcall(function()
        ...
        local script = mod_path:gsub('[/\\]+$', '')..'/server/start-recorder.ps1'
        ...
        assert(present, 'server/start-recorder.ps1 is missing from the mod folder; reinstall the mod')
        present:close()
