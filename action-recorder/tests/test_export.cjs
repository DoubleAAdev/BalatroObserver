...
(async()=>{
 ...
 try{
  ...
  assert.deepEqual(exported.actions[1].cards.map(c=>c.card),['Ace of Spades','King of Spades']);
  assert.equal(exported.cards['Ace of Spades'].edition,undefined);
  assert.equal(exported.cards['Ace of Spades'].stickers,undefined);
  assert.equal(exported.cards['Ace of Spades'].perma_bonus,undefined);
  assert.ok(JSON.stringify(exported.cards).length < JSON.stringify(Object.assign({},...journal.trim().split('\n').slice(1).map(line=>JSON.parse(line).cards))).length);
