...
(async()=>{
 ...
 try{
  ...
  assert.match(exported,/2\. discard \[hand\]: Ace of Spades \[card \d+, slot 2\]; King of Spades \[card \d+, slot 3\]/);
  assert.match(exported,/changed: identity=Queen of Spades/);
  assert.match(exported,/face-down card \(slot 3\)/);
