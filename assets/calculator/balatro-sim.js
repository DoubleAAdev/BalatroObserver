...
class Hand {
  ...
  constructor({
    ...
    if(hands) {
      ...
      for(let i = 0; i < this.hands.length; i++) {
        if(this.hands[i][PLAYED_THIS_ROUND]) {
  ...
  triggerJoker(joker, j) {
    ...
    switch(joker[JOKER]) {
      ...
      case 89:
        // Bootstraps
        this.mult = bigAdd((joker[VALUE] + Math.floor(this.earnedMoney / 5)) * 2, this.mult);
        break;
      case 102:
        // Blackboard
        if(this.compiledValues[j]) {
      ...
      case 147:
        // Bull
        this.chips += 2 * (joker[VALUE] + this.earnedMoney) + this.jokersExtraValue[j];
        break;
      case 150:
        // Flash Card
  ...
  triggerCard(card, retrigger = false) {
    ...
    this.earnedMoney += luckyMoney * 20;
    ...
    if(notStone) {
      for(let j = 0; j < this.jokers.length; j++) {
        ...
        switch(joker[JOKER]) {
          ...
          case 35:
            // Golden Ticket
            if(card[ENHANCEMENT] === GOLD || (this.MidasMaskas && isFace)) {
              this.earnedMoney += 4;
            }
            break;
          case 38:
            ...
            if(isFace) {
              switch(this.randomMode) {
                ...
                case 1:
                  ...
                  break;
                case 2:
                  if(Math.random() < 0.5 * this.chanceMultiplier) {
                    this.earnedMoney += 2;
                  }
                  break;
              }
  ...
  compileCards() {
    ...
    for(let j = 0; j < this.jokers.length; j++) {
      ...
      switch(joker[JOKER]) {
        ...
        case 105:
          // DNA
          if(this.cards.length === 1 && this.firstHand) {
...
// Observer CommonJS entry point; engine otherwise unchanged.
if(typeof module === "object") module.exports = {Hand, handChips};
