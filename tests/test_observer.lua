...
_G.G = G
SMODS = {current_mod = {id = 'BalatroObserver', version = '0.7.2'}, load_file = function(file) return loadfile(file) end}
...
assert(BalatroObserver.version == '0.7.2')
assert(BalatroObserver.snapshot().mod_version == '0.7.2')
assert(writes['balatro_observer/state-1.json']:find('0.7.2', 1, true))
fail = true
Game:update(0.2)
