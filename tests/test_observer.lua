...
_G.G = G
SMODS = {current_mod = {id = 'BalatroObserver', version = '2.0.2'}, load_file = function(file) return loadfile(file) end}
...
assert(BalatroObserver.version == '2.0.2')
assert(BalatroObserver.snapshot().mod_version == '2.0.2')
assert(writes['balatro_observer/state-1.json']:find('2.0.2', 1, true))
fail = true
Game:update(0.2)
