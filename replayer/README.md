...
A round that ends earlier or later here than it did in the log leaves the game on a screen the log's next inputs will never be answered from - the log plays another hand while the game is already in the shop. The replay recognises that within ten seconds, skips to the first input the screen can serve, and counts the rest as one difference naming how many went.
