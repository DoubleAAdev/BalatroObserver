-- Windows' native Open dialog. Paths are returned as UTF-8; no shell is involved.
return function()
    assert(love.system.getOS()=='Windows','Drop a .log onto Balatro on this platform')
    ...
    local filter=wide('Multiplayer logs (*.log)\0*.log\0All files (*.*)\0*.*\0\0')
    local title=wide('Replayer - Select a Multiplayer log')
    local options=ffi.new('BOBS_OPENFILENAMEW')
    options.lStructSize=ffi.sizeof(options);options.hwndOwner=user.GetActiveWindow()
    options.lpstrFilter=filter;options.nFilterIndex=1;options.lpstrFile=filename;options.nMaxFile=32768
    options.lpstrTitle=title
    -- Explorer-style, existing files/paths only, preserve the game's current directory.
