...
**Replayer** is available in Observer's config. Use **Load Log** to choose the Lovely log of a Multiplayer game, then **Start Replay** from the main menu. The game joins a copy of the recorded lobby and executes the log's actions in order - only those, each once - stopping at the first one it cannot do, and Action Recorder records the result. See [Replayer use and limits](replayer/README.md).
